...
file_json = "./Vts/config.json"   # 秘钥保存文件位置

async def init(websocket):
    with open(file_json, "r") as config_file:     # 读文件
        data = json.load(config_file)
        if data['authenticationkey'] == "":     # 判断是否没有令牌
            authtoken = await vtube_token(websocket)
            confirm = await vtube_plugin(websocket, authtoken)
...
async def vtube_read(websocket, sport_file):
    ...
    while 1:
        ...
        parameter_values = await vtube_request(websocket)
...
async def vtube_sportout(websocket, file):
    ...
    for i in range(len(data)):
        await vtube_control(websocket, data[i])
        await asyncio.sleep(0.02)
