...
def waiting_sport(time, amplitude = 2, frequency = 0.6, phase_shift = np.pi / 2, shifting = 0):
    num = amplitude * np.sin(2 * np.pi * frequency * time + phase_shift) + shifting
...
async def waiting_model(time):
    ...
    FaceAngleX = waiting_sport(time,amplitude=1.5, frequency = 0.25, phase_shift = np.pi)
    Brows = waiting_sport(time,amplitude=0.025, frequency = 0.5, phase_shift = np.pi, shifting=0.55)
    EyeOpenLeft = EyeOpenRight = waiting_sport(time, amplitude=10, frequency = 0.5, phase_shift = np.pi, shifting = 10)
    EyeRightX = waiting_sport(time, amplitude=0.1, frequency = 0.5, phase_shift = np.pi)
    ...
    while True:
        if action_file != "":
            ...
            for i in range(len(data)):
                Brows = waiting_sport(time,amplitude=0.025, frequency = 0.5, phase_shift = np.pi, shifting=Brows_shifting)
